...
clock_tab = ttk.Frame(tabs_control)
alarm_tab = ttk.Frame(tabs_control)
frame_a = ttk.Frame(alarm_tab)
frame_a.place(relx=.5, rely=.3, anchor='center')
stopwatch_tab = ttk.Frame(tabs_control)
timer_tab = ttk.Frame(tabs_control)
timer_tab.place(relx=.5, rely=.5, anchor='c')
frame_t = ttk.Frame(timer_tab)
...
time_label = ttk.Label(clock_tab, font='calibri 40 bold', foreground='black')
time_label.place(relx=.5, rely=.3, anchor='center')
date_label = ttk.Label(clock_tab, font='calibri 40 bold', foreground='black')
...
a_hrs = ttk.OptionMenu(frame_a, alarm_hour, *hours)
...
a_mins = ttk.OptionMenu(frame_a, alarm_minutes, *minutes)
...
a_sec = ttk.OptionMenu(frame_a, alarm_second, *seconds)
a_sec.pack(side=LEFT)
alarm_time2 = StringVar(alarm_tab)
ap = ('AM', 'PM')
alarm_time2.set(ap[0])
aps = ttk.OptionMenu(frame_a, alarm_time2, *ap)
aps.pack(side=LEFT)
set_alarm_Button = ttk.Button(alarm_tab, text="Set Alarm", command=alarm)
set_alarm_Button.place(relx=.5, rely=.5, anchor='center')
alarm_status_label = ttk.Label(alarm_tab, font='calibri 15 bold')
alarm_status_label.place(relx=.5, rely=.7, anchor='center')
stopwatch_label = ttk.Label(stopwatch_tab, font='calibri 40 bold', text='Stopwatch')
stopwatch_label.place(relx=.5, rely=.1, anchor='center')
stopwatch_start = ttk.Button(stopwatch_tab, text='Start', command=lambda: stopwatch('start'))
stopwatch_start.place(relx=.5, rely=.3, anchor='center')
stopwatch_stop = ttk.Button(stopwatch_tab, text='Stop', state='disabled', command=lambda: stopwatch('stop'))
stopwatch_stop.place(relx=.5, rely=.6, anchor='center')
stopwatch_reset = ttk.Button(stopwatch_tab, text='Reset', state='disabled', command=lambda: stopwatch('reset'))
stopwatch_reset.place(relx=.5, rely=.9, anchor='center')
timer_hour = StringVar(frame_t)
timer_hour.set(hours[0])
t_hrs = ttk.OptionMenu(frame_t, timer_hour, *hours)
t_hrs.pack(side=LEFT)
timer_minutes = StringVar(frame_t)
timer_minutes.set(minutes[0])
t_mins = ttk.OptionMenu(frame_t, timer_minutes, *minutes)
t_mins.pack(side=LEFT)
timer_second = StringVar(frame_t)
timer_second.set(seconds[0])
t_sec = ttk.OptionMenu(frame_t, timer_second, *seconds)
t_sec.pack(side=LEFT)
timer_label = ttk.Label(timer_tab, font='calibri 40 bold', text='Timer')
timer_label.place(relx=.5, rely=.3, anchor='center')
timer_start = ttk.Button(timer_tab, text='Start', command=lambda: timer('start'))
timer_start.place(relx=.5, rely=.5, anchor='center')
timer_stop = ttk.Button(timer_tab, text='Stop', state='disabled', command=lambda: timer('stop'))
timer_stop.place(relx=.5, rely=.7, anchor='center')
timer_reset = ttk.Button(timer_tab, text='Reset', state='disabled', command=lambda: timer('reset'))
